//! Stub implementation of the secrets generators component.
//! Generates secrets, passwords, and cryptographic material on demand.
//! This is a placeholder — full generation logic to be implemented.
...
impl schema_core_api::Guest for Generators {
    fn describe() -> Vec<u8> {
        ok(json!({
            "provider_type": "generators",
            "id": "greentic.secrets.generators",
            "capabilities": ["generate_secret", "generate_password", "generate_key"],
            "ops": ["generate_secret", "generate_password", "generate_key"],
        }))
    }
    ...
    fn invoke(op: String, _input_json: Vec<u8>) -> schema_core_api::InvokeResult {
        match op.as_str() {
            "generate_secret" | "generate_password" | "generate_key" => ok(json!({
                "status": "ok",
                "value": null,
                "message": "stub: generation not yet implemented",
            })),
