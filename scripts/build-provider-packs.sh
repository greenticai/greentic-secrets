...
components_registry="ghcr.io/greenticai/components"
...
PY
  ...
  greentic-pack resolve --in "${staging}" --lock "${LOCK_FILE}"
  greentic-pack build \
    --in "${staging}" \
    --lock "${LOCK_FILE}" \
...
greentic-pack resolve --in "${bundle_staging}" --lock "${LOCK_FILE}"
greentic-pack build \
  --in "${bundle_staging}" \
  --lock "${LOCK_FILE}" \
